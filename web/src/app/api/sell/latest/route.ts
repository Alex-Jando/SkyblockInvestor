  ...
  try {
      ...
      return NextResponse.json(
        {
          ...
          model_version: null,
          notes: "No sell signals available yet.",
          items: []
        },
        { status: 200 }
      );
    ...
    const itemsResult = await pool.query(
      ...
      where bi.basket_id = $1
        and bi.action = 'SELL'
      order by bi.expected_return asc, bi.confidence asc
      `,
    ...
    console.error("GET /api/sell/latest failed", error);
    return NextResponse.json(
      { error: "Failed to load latest sell list." },
      { status: 500 }
    );
  }
}
