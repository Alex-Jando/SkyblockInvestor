...
# Cold-start expected_return_H heuristic:
# expected_return_H = 0.40*return_7d + 0.30*return_14d + 0.20*imbalance
#                     - 0.15*spread_pct - 0.25*volatility_30d
HEURISTIC_WEIGHTS = {
    "w1_return_7d": 0.40,
    "w2_return_14d": 0.30,
    "w3_imbalance": 0.20,
    "w4_spread": 0.15,
    "w5_volatility_30d": 0.25,
}
...
def _heuristic_expected_return(frame: pd.DataFrame) -> np.ndarray:
    w = HEURISTIC_WEIGHTS
    return (
        w["w1_return_7d"] * frame["return_7d"].fillna(0.0).to_numpy()
        + w["w2_return_14d"] * frame["return_14d"].fillna(0.0).to_numpy()
...
def predict_horizon_signals(
    ...
    for horizon in HORIZONS:
        ...
        if model is None:
            expected_return = _heuristic_expected_return(frame)
        else:
            expected_return = model.predict(x)

        vol_30d = frame["volatility_30d"].fillna(0.02).abs().to_numpy()
        spread = frame["spread_pct"].fillna(0.0).to_numpy()
        liquidity = frame["liquidity_score"].fillna(0.0).to_numpy()

        confidence = _sigmoid(expected_return / (vol_30d + 0.01))
        spread_penalty = np.clip(spread / max(spread_max, EPS), 0.0, 1.0)
        liquidity_factor = np.clip(liquidity / max(liquidity_min, EPS), 0.0, 2.0)

        confidence *= (1.0 - 0.45 * spread_penalty)
        confidence *= np.clip(liquidity_factor / 1.5, 0.35, 1.0)
